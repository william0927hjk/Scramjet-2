...
await Promise.all([
    fs.promises.writeFile(path.join(serializedTriesDir, "icann.js"), `export default ${JSON.stringify(serializedIcannTrie)};`),
    fs.promises.writeFile(path.join(serializedTriesDir, "private.js"), `export default ${JSON.stringify(serializedPrivateTrie)};`),
    fs.promises.writeFile(path.join(serializedTriesDir, "info.json"), JSON.stringify({ updatedAt: new Date() })),
]);
